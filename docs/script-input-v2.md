...
当前应用已支持 V1/V2 双读：导入、编辑和 AI 生成接口接受两种版本；V2 通过兼容投影继续供现有引擎和 AI 使用，V1 数据无需迁移即可继续开局。

页面端优先使用 V2 的结构化字段渲染：剧本详情、角色卡、线索卡和 DM 真相使用统一内容块组件，时间线使用固定时间列与内容列，避免把长文本直接拼接在同一段中造成重叠。公开 API 只返回公开切片，私卡和 `truth` 仍按座位 / DM 鉴权返回。

新剧本应直接提交 V2；旧剧本可先用迁移工具生成 V2，再根据命令输出的警告人工补齐动机、证据链和无法识别的时间信息。
