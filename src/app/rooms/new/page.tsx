...
export default function NewRoomPage() {
  return (
    <Suspense fallback={<p className="text-paper-400">加载中…</p>}>
      <NewRoomContent />
    </Suspense>
  );
}
...
function NewRoomContent() {
  ...
  if (!doc) return <p className="text-paper-400">加载剧本…</p>;
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <label className="block text-sm text-paper-300">
        剧本
        <select value={scriptId} onChange={(e) => setScriptId(e.target.value)} className="mt-1 w-full rounded-lg border border-gold-400/25 bg-ink-950 px-3 py-2 outline-none focus:border-gold-400">
          {scripts.map((s) => (
      ...
      {doc && (
        <div className="rounded-xl border border-gold-400/12 bg-ink-900/60 p-5 text-sm text-paper-300">
          <span className="font-medium text-paper-200">{doc.title}</span> · {doc.intro}
        </div>
      )}
      ...
      <div className="space-y-2">
        ...
        {seats.map((seat, i) => (
          <div key={i} className="flex flex-wrap items-center gap-3 rounded-lg border border-gold-400/12 bg-ink-900/50 px-4 py-3">
            <span className="w-14 text-sm text-paper-400">座位 {i + 1}</span>
            <select
              aria-label={`座位 ${i + 1} 类型`}
              value={seat.kind}
            ...
            {seat.kind !== "empty" && (
              <select
                aria-label={`座位 ${i + 1} 角色`}
                value={seat.characterId ?? ""}
                onChange={(e) => setSeats((s) => s.map((x, j) => (j === i ? { ...x, characterId: e.target.value || null } : x)))}
                className="flex-1 rounded-lg border border-gold-400/25 bg-ink-950 px-2 py-1.5 text-sm outline-none focus:border-gold-400"
              >
            ...
            {seat.kind !== "empty" && <span className="text-xs text-paper-400">→ {charName(seat.characterId ?? previewIds[i] ?? null)}</span>}
          </div>
        ))}
      </div>

      <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-secret-400/30 bg-secret-400/5 p-4 text-sm">
        <input type="checkbox" checked={humanDm} onChange={(e) => setHumanDm(e.target.checked)} className="mt-0.5" />
        <span>
          <span className="font-medium text-paper-100">启用真人 DM</span>
          <span className="mt-1 block text-xs text-paper-400">开局后由一位真人负责旁白、催促和跳过回合，并可查看完整真相。</span>
        </span>
      </label>

      <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-gold-400/12 bg-ink-900/60 p-4 text-sm">
        <input
          type="checkbox"
        ...
        />
        <span>
          <span className="font-medium text-paper-200">真人操作不限时</span>
